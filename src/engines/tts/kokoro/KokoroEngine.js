...
// Each free-tier voice gets its own independent daily count — UsageStore
// is keyed by arbitrary string, so "af_sky"/"bf_emma"/"bm_george" don't
// share a pool.
const FREE_KOKORO_VOICE_IDS = VOICE_LIST.filter((v) => v.tier === "free").map(
  (v) => v.id,
);
...
class KokoroEngine {
  ...
  _ensureChild() {
    ...
    this.child = utilityProcess.fork(
      path.join(__dirname, "kokoroWorker.js"),
      [],
      {
...
module.exports.FREE_KOKORO_VOICE_IDS = FREE_KOKORO_VOICE_IDS;
